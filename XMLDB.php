<?php
 ...
 class Database {
      ...
      */
     public static function factory($filename, $id = NULLL, $tableName = NULL)
     {
         Data::newInstance();
         $db = new Database();
         $db->_data = new Data();
         $db->set_informations($filename, $id);
         $db->rootXml = simplexml_load_file($db->file);
	 if($tableName && $db->searchTables($tableName)){
		$db->chooseTable($tableName,$id); 
	 }
         return $db;
     }
...
<Database name="'.$filename.'">
      ...
      * @param string $filename name of file without extension
      */
     private function searchTables($tableName)
     {
	     if($table = $this->rootXml->xpath("//table[@name='".$tableName."']")){
	    	return $table[0];
	     }else{
	    	return false;
	     }
	...
	{
		if(!$this->searchTables($tableName)){
			$database = $this->rootXml;
			$table = $database->addChild("table");
			$table->addAttribute("name",$tableName);
			$table->addAttribute("lastID",0);
		}else{
	...
	{
		if($this->searchTables($tableName)){
			$tableXml = dom_import_simplexml($this->searchTables($tableName));
			$tableXml->parentNode->removeChild($tableXml);
		}else{
			throw new Exception('The Deleting Table not exists');
		}
	}
		
	public function chooseTable($tableName,$id=NULL)
	{
		if($tableXml = $this->searchTables($tableName)){
			$this->xml = $tableXml;
     ...
     private function _edit()
     {
         ...
         foreach (get_object_vars($data) as $name => $value)
         {
             ...
             //$row[0]->field['type'] = $type; //fix wrong type bug in edit mode, don't figure out reason.
             if ($name != 'id')
                 $row[0]->field[$i] = $value;
             $i++;
         }
